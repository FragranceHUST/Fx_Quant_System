...
PeriodCollection::PeriodCollection(const char *instrument, const char *timeframe, int precision,  bool alive, IPriceUpdateController *controller) :
...
void PeriodCollection::handleOffer(IOffer *offer)
{
    ...
    // std::cout << "mTimeframeUnit: " << mTimeframeUnit << ", mTimeframeLength: " << mTimeframeLength << ", mTradingDayOffset: " << mTradingDayOffset << ", start: " << start << std::endl;
    ...
    else 
    {
        ...
        if (fabs(period->getTime() - start) < HALF_SECOND) // period->getTime() == start
        {
            ...
            std::cout << "mLastMinute: " << mLastMinute << ", mLastMinuteVolume: " << mLastMinuteVolume << ", offerVolume: " << offer->getMinuteVolume() << std::endl;
        ...
        {
            // Update minute data when new period start
            notifyLastPeriodUpdated();
...
int PeriodCollection::getPrecision()
{
    return mPrecision;
}
